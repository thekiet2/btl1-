...
void RenderWindow::render(Entity& p_entity)
{
	...
	for (int i = p_entity.getSize() - 1; i >= 0; i--)
	{
		...
		SDL_Rect dst;
		dst.x = p_entity.getX() + p_entity.getAnisetX(i);
		dst.y = p_entity.getY() + p_entity.getAnisetY(i);
...
void RenderWindow::render(Mons &mons) {
    int textureWidth = mons.getWidth();
    int textureHeight = mons.getHeight();
    int currentX = mons.getX();
    int currentY = mons.getY();
    int xCenter = currentX - textureWidth / 2;
    int yCenter = currentY - textureHeight / 2;

    SDL_Rect dstRect = { xCenter, yCenter, textureWidth, textureHeight };
    SDL_RenderCopy(renderer, mons.getTexture(), NULL, &dstRect);
}
